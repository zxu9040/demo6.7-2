...
const noteFilters = [
  ...
  { key: "interest_circle", label: "同好圈可见" },
  { key: "anonymous_public", label: "匿名公开" },
  { key: "public", label: "公开可见" },
  { key: "share_card_only", label: "分享卡片" }
];
const visibilityOptions = [
  ...
  },
  {
    ...
    drawerLabel: "同好圈可见",
    text: "只展示给选择相同兴趣标签的人。",
    hint: "这条笔记会出现在相关兴趣圈层中。",
    action: "发布到同好圈",
    toast: "已发布到同好圈"
  },
  {
    key: "anonymous_public",
    label: "匿名公开",
    drawerLabel: "匿名公开",
    text: "隐藏昵称后公开，只保留内容和标签。",
    hint: "公开前会隐藏你的昵称。",
    action: "匿名公开",
    toast: "已匿名公开"
  },
  ...
  },
  {
    ...
    label: "仅生成分享卡",
    drawerLabel: "分享卡片",
    text: "生成一张分享图，是否发出去由你决定。",
    hint: "这里只生成分享预览，不会自动发布。",
    action: "生成分享卡片",
    toast: "已生成分享卡片"
  }
];
const mockFriends = ["小林", "阿周", "室友", "旅行搭子", "学习搭子"];
const circleTags = ["一人出行", "低预算生活", "租房改造", "新手拍照", "情绪回血", "周末灵感", "学习入门", "穿搭记录"];
...
function runGeneration(mode) {
  ...
  window.setTimeout(() => {
    ...
    appState.generatedPlans = generateInspirationPlans(input, appState.selectedMood, appState.selectedCategory, notes, mode);
...
function generateInspirationPlans(userInput, selectedMood, selectedCategory, notes, adjustType) {
  const categoryKey = inferCategory(userInput, selectedCategory);
  const moodTags = inferMoodTags(selectedMood, adjustType);
...
function inferMoodTags(selectedMood, adjustType) {
  const base = {
    "我有点累，想轻松一点": ["低电量", "想独处"],
    "我想找点新鲜感": ["想找新鲜感", "轻量探索"],
    "我想省钱但过得有趣": ["想省钱", "小预算"],
  ...
  return extra ? Array.from(new Set([...base, extra])) : base;
}
...
function pickNotes(notes, template, index) {
  ...
  return list.slice(index, index + 2).length ? list.slice(index, index + 2) : list.slice(0, 2);
}
...
function renderPlanCard(plan) {
  ...
  return `
    <article class="plan-card" data-plan-id="${plan.id}">
      ...
      <div class="plan-body">
        <div class="mini-tags">${plan.moodTags.map((tag, index) => `<span class="tag ${tagClass(index)}">${tag}</span>`).join("")}</div>
        <h3>${escapeHtml(plan.title)}</h3>
        <div class="meta-line"><span>${escapeHtml(plan.suitableFor)}</span></div>
        <div class="meta-line">
          <span class="tag green">预计花费 ${escapeHtml(plan.cost)}</span>
          <span class="tag purple">${plan.replicateScore}/5 可复刻</span>
        </div>
        ...
        <div class="composer-actions">
          <button class="secondary-button" type="button" data-toggle-plan="${plan.id}">${plan.isExpanded ? "收起步骤" : "展开步骤"}</button>
          <button class="mini-button" type="button" data-save-plan="${plan.id}">放进灵感盒子</button>
        </div>
...
function renderPlanDetails(plan, checklist) {
  return `
    <div class="detail-panel">
      ...
      <section class="detail-section">
        <h4>小红书上大家怎么做</h4>
        <div class="xhs-grid">${plan.relatedNotes.slice(0, 2).map(renderXhsNote).join("")}</div>
      </section>
...
function renderExperienceComposer(plan) {
  ...
  return `
    <section class="note-composer" data-note-plan="${plan.id}">
      ...
      ${draft.visibility === "share_card_only" ? `<div class="share-preview"><strong>分享卡片预览</strong><p>${escapeHtml(draft.text || "做了一点点也算数。")}</p><span class="tag purple">${escapeHtml(plan.title)}</span></div>` : ""}
...
function renderVisibilitySelectors(planId, draft) {
  ...
  if (draft.visibility === "interest_circle") {
    return `<div class="selector-panel"><h4>选择圈层</h4><div class="pill-row">${circleTags.map((name) => `<button class="pill ${draft.visibleToCircles.includes(name) ? "is-active" : ""}" type="button" data-circle="${planId}" data-value="${escapeAttr(name)}">${name}</button>`).join("")}</div></div>`;
  }
  return "";
}
...
function adjustPlans(adjustType) {
  ...
  appState.generatedPlans = generateInspirationPlans(appState.currentInput, appState.selectedMood, appState.selectedCategory, notes, adjustType)
  ...
  dom.savedPlansList.innerHTML = appState.savedPlans.map((plan) => `
    <article class="drawer-item saved-item">
      ...
      <div>
        ...
        <div class="mini-tags">${plan.moodTags.map((tag) => `<span class="tag">${escapeHtml(tag)}</span>`).join("")}</div>
...
function renderHistory() {
  ...
  dom.historyList.innerHTML = appState.historyRecords.map((record) => `
    <article class="drawer-item">
      <div class="meta-line"><span class="tag blue">${escapeHtml(record.selectedCategory)}</span><span>${formatDate(record.createdAt)}</span></div>
      <h3>${escapeHtml(record.input)}</h3>
      <p>${escapeHtml(record.judgement)}</p>
      <button class="mini-button" type="button" data-restore-history="${record.id}">再看这组</button>
    </article>
  `).join("");
}
...
function setVisibility(planId, value) {
  ...
  draft.visibleToCircles = value === "interest_circle" ? draft.visibleToCircles : [];
  persistState();
  renderResults();
}
...
function saveExperienceNote(planId) {
  ...
  const note = {
    ...
    isAnonymous: draft.visibility === "anonymous_public",
    shareCardData: draft.visibility === "share_card_only" ? { title: plan.title, text: draft.text.trim() || "做了一点点也算数。" } : null,
    createdAt: new Date().toISOString(),
